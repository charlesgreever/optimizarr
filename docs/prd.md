...
207. As an operator, I want that webhook to inspect the new or changed file and not enqueue from the HTTP handler, so that Connect Test stays harmless. When Queue new Arr imports is on, inspect of a new or upgraded file may queue a sidecar; Keep still replaces the library file.
...
- New imports are inspected automatically. An optional setting (`queueNewImports`, default off) may enqueue a **sidecar** after inspect of a file first seen or changed after the setting was turned on. Keep still replaces the library file. Direct write is not used for these jobs. Turning the setting on does not queue the existing library. Radarr and Sonarr Connect may POST `/api/hooks/arr` on import, upgrade, or rename so sync does not wait for the 15-minute timer. The hook is token-authenticated (not the login cookie). A Connect Test returns 200. The hook handler itself does not enqueue.
...
- Web UI / HTTP: bounded Movies, Suggestions, Queue, Review, Errors, and History pages; Series summaries with healthy and suggestion counts and no path; suggestion rows, error rows, and job rows include `href` when the library item is known; episodes for one expanded show; search `q`; enqueue; Keep/Discard; Home payload includes Status; settings (credentials, Arr enabled, widget key mint). `GET`/`POST /api/integrations` require a session. Job pages expose write mode and size vs quality; `GET /api/jobs/:id/logs` is bounded. Seeded list tests assert first-page cardinality and continuation metadata. Large-library tests assert a bounded first response and no episode payload before expansion. Polling cleanup when a request is still in flight. Force/stereo HTTP: unreadable is not 200; stereo no-op is not 200. Unauthenticated integrations writes are 401.
