...
Work is asynchronous. Enqueue, Cancel, Keep, and navigation return at once. ffmpeg handles probes, hardware video encode, and creating the AAC stereo stream. MKVtoolnix (`mkvmerge`) muxes tracks, subtitles, and that stereo stream because that mux is faster than ffmpeg. Output is a sidecar on a separate NAS review path. I Keep or Discard each result. Keep replaces the original, deletes the old file, asks Radarr or Sonarr to refresh, and notifies Plex and Jellyfin. Hardware encode failures fail the job and tell me. There is no software fallback, no auto-optimize, no multi-segment encode, and no storage-aware NAS copy layer.
...
106. As a library owner, I want remux output to be Matroska, even if the source was MP4, so that MKVtoolnix can own the container. Keep may change the extension; the Arr refresh updates the path.
...
137. As a library owner, I want container restart to resume or re-queue unfinished work without half-writing a library file, so that a reboot is safe.
...
- Track policy: keep all audio and subtitles tagged in the preferred language; suggest stripping other languages; drop untagged tracks except a lone untagged dialogue track. Original surround stays when stereo is added. Added stereo is AAC 2.0. Stereo is auto-suggested for Atmos or layouts above 5.1, and is always offered as a manual action when the file is not already stereo.
- Tool split: ffmpeg (and ffprobe) for inspect, hardware video encode, and creating the AAC stereo stream. MKVtoolnix `mkvmerge` for muxing: drop non-preferred tracks, attach the stereo stream, copy chapters and attachments. Track-only jobs do not video-encode. When a job needs track cleanup and a codec or size encode, `mkvmerge` first, then ffmpeg on that working file. Remux output is Matroska. Keep may change `.mp4` to `.mkv`. The container image includes ffmpeg and MKVtoolnix. Invoke both with argument arrays (`execFile`), never a shell (ENG-08).
...
- Hardware encode only (CUDA or VAAPI from devices passed into the container). Hardware failure fails the job and is shown. No automatic software fallback (ENG-05).
