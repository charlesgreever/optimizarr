...
- List reads use batched public read models instead of loading related state once per row. Movies, Suggestions, Queue, Review, Errors, and History return bounded pages with continuation metadata. Series returns show summaries first (episode total, healthy count, suggestion count) and fetches one show’s episodes when the operator expands it. Suggestion rows include an `href` to the title page. Home and the Homepage widget use aggregate counts plus a bounded recent-activity query. The browser may retain pages and expanded-show results until Refresh invalidates them.
...
- Web UI / HTTP: bounded Movies, Suggestions, Queue, Review, Errors, and History pages; Series summaries with healthy and suggestion counts and no path; suggestion rows include `href`; episodes for one expanded show; search `q`; enqueue; Keep/Discard; Home payload; settings. Seeded list tests assert first-page cardinality and continuation metadata. Large-library tests assert a bounded first response and no episode payload before expansion. Polling cleanup when a request is still in flight. Force/stereo HTTP: unreadable is not 200; stereo no-op is not 200.
