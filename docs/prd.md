...
- Web UI / HTTP: bounded Movies, Suggestions, Queue, Review, Errors, and History pages; Series summaries with healthy and suggestion counts and no path; suggestion rows and error rows include `href` when the library item is known; episodes for one expanded show; search `q`; enqueue; Keep/Discard; Home payload; settings. Seeded list tests assert first-page cardinality and continuation metadata. Large-library tests assert a bounded first response and no episode payload before expansion. Polling cleanup when a request is still in flight. Force/stereo HTTP: unreadable is not 200; stereo no-op is not 200.
