...
- Optimize runner: review-path output (never library folder), `mkvmerge` used for track mux and opted-in MP4 conversion, ffmpeg used for hardware video and stereo AAC creation, MP4 remux before transcode, hardware failure becomes a failed job with a user-visible error, integrity/duration checks, temp cleanup, disk-space preflight, bounded logs, real phase and progress on the public job payload. Accept MKVToolNix exit code 1 as a completed mux with warnings, then probe the output. Treat exit code 2 as failure and preserve its stdout or stderr diagnostic. No live NAS or GPU.
