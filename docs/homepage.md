...
[Homepage](https://github.com/gethomepage/homepage) can poll Polisharr without a browser login cookie.
...
1. **Widget key** (recommended). In Polisharr Settings you can create a key. Homepage sends it as `X-Api-Key` or `Authorization: Bearer …`. The key is stored as a SHA-256 hash.
2. **Environment.** Set `POLISHARR_WIDGET_KEY` on the Polisharr container to the same string Homepage sends.
...
- Arr Stack:
    - Polisharr:
        ...
        widget:
          ...
          headers:
            X-Api-Key: "{{HOMEPAGE_VAR_POLISHARR_KEY}}"
          mappings:
            - field: status
              label: Status
            - field: queued
